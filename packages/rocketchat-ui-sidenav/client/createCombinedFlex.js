Template.createCombinedFlex.helpers({
	...
	unjoinableSwitchHidden() {
		if (!RocketChat.authz.hasAllPermission('create-c')) {
			return 'hidden';
		}
		if (Template.instance().privateGroup.get()) {
			return 'hidden';
		} else {
			return '';
		}
...
Template.createCombinedFlex.events({
	...
	'click .save-channel'(e, instance) {
		...
		const unjoinable = privateGroup ? false : instance.find('#unjoinable').checked;
		...
		if (!err) {
			return Meteor.call(createRoute, name, instance.selectedUsers.get(), readOnly, {}, unjoinable, function(err, result) {
				if (err) {
					console.log(err);
