...
Template.channelSettings.onCreated ->
	...
	@settings =
		...
		joinCode:
			type: 'text'
			label: 'Password'
			canView: (room) => false # room.t is 'c' and RocketChat.authz.hasAllPermission('edit-room', room._id)
			canEdit: (room) => false # RocketChat.authz.hasAllPermission('edit-room', room._id)
			save: (value, room) ->
